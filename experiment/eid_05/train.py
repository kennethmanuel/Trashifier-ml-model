...
def main():
    ...
    def preprocess_ds(ds):
        def preprocess_image(images, labels):
            preprocessed_images = tf.keras.applications.mobilenet_v2.preprocess_input(
                images)
    ...
    plot_acc_loss_fine_tune(history, fine_tuned_history, 'fine_tune_acc_loss.png')
    predictions, labels = get_predictions_labels(test_ds, fine_tuned_model)
    generate_classification_report(labels, predictions, 'fine_tune_classification_report.csv')
    plot_confusion_matrix(labels, predictions, class_names, 'fine_tune_confusion_matrix.png')
